...
rankの低いhandはFold equityを作るBluff、中程度のrankを持つhandはshowdown/Check、rank 38〜39はThin value、44以上は
...
#### hand群・street別に比較するactionを整理

| street・hand群 | 主な役割 | 比較するaction |
|---|---|---|
| 1st、rankが低いhand | 即時Bluffと次streetへ残すBluff | Check / 33% / 100% Pot |
| 1st、rankが中程度のhand | showdown valueの保持 | 主にCheck / Thin value |
| 1st、rankが高いhand | ValueとChecking-range防御 | Check / 33% / 100% Pot |
| 2nd、rankが低いhand | 最終streetのBluff | Check / 100% Pot |
| 2nd、rankが高いhand | 最終streetのValue | Check / 100% Pot |

1st-street actionは、その場の利得だけでなく、action後に到達する2nd-street戦略のEVを含めて比較します。

showdown EQ自体はone-street版と同じ$`EQ(r)=(r-0.5)/50`$です。しかし1st-street Action EVには、
現在のFold/Call/Raiseだけでなく、次streetで最適にプレイした場合のEVも入ります。例えばrank $`r`$のhandが
