...
限定した有限ゼロサムゲームです。ランクの強さは `A > K > Q` です。
...
初期ポットは常に1で、設定からは変更できません。utilityは初期ポットを両者が0.5ずつ
拠出したと考えた純利益です。
...
\[
EV_{IP}=\frac{s}{2(1+s)},\qquad
EV_{OOP}=-\frac{s}{2(1+s)}
\]

デフォルトの \(s=1\) では、IP(Q)のbluff率とOOPのCall率がともに50%、
`IP EV = +0.25`、`OOP EV = -0.25` です。

例えば `oop_stack=2`, `ip_stack=3` なら \(s=2\) なので、IP(Q)のbluff率は
2/3、OOPのCall率は1/3、IP EVは1/3になります。
